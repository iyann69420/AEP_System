...
function moveToRestoreTable($recordId, $mysqli) {
    ...
    $result = $mysqli->query("SELECT * FROM unitlogs2 WHERE id = '$recordId'");
...
function deleteRecord($recordId, $mysqli) {
    $recordId = $mysqli->real_escape_string($recordId);
    $mysqli->query("DELETE FROM unitlogs2 WHERE id = '$recordId'");
    echo '<script>alert("Delete record sucessfully");</script>';
}

function logoutAllRecords($mysqli) {
    $result = $mysqli->query("UPDATE unitlogs2 SET time_out = NOW() WHERE time_out = '0000-00-00 00:00:00'");
...
$sql = "SELECT id, First_Name, Last_Name, section,year, student_number, name, Time, time_out FROM unitlogs2 ORDER BY name DESC";
...
function downloadAsPDF(element) {
    html2canvas(element, { scrollY: -window.scrollY }).then(function (canvas) {
        ...
        pdf.addImage(canvas.toDataURL(), 'PNG', 0, 0, canvas.width / 6.39, canvas.height / 10);
        pdf.save('table_pdf.pdf');
    }).then(function () {
        // After PDF download is complete, delete records
        deleteAllRecords();
    });
}
...
function downloadAsImage(element) {
    html2canvas(element, { scrollY: -window.scrollY }).then(function (canvas) {
        ...
        // After image download is complete, delete records
        deleteAllRecords();
    });
}

function deleteAllRecords() {
    // Implement your logic to delete all records here
    console.log('Deleting all records...');
}

        function deleteAllRecords() {
            var xhr = new XMLHttpRequest();
            xhr.open('POST', 'move_records_to_restores.php', true);
        ...
        function logoutRecord(recordId) {
            var xhr = new XMLHttpRequest();
            xhr.open('POST', 'logout_record_303.php', true);
            xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4 && xhr.status == 200) {
                    console.log(xhr.responseText);
    ...
    function redirectToArchives() {
        window.location.href = "/TRACK-WISE/faculty2/pages/unitsave303.php";
    }

 
</script>
<script>
    function moveTo303Restore() {
        if (confirm("Are you sure you want to move all records to 302restore?")) {
            var xhr = new XMLHttpRequest();
            xhr.open('POST', 'clear_table_303.php', true);
            xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4 && xhr.status == 200) {
                    console.log(xhr.responseText);
...
<body class="bg-content">
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr style="background-color: <?php echo $backgroundColor; ?>;">
                                    ...
                                    <td>
                                        <a href="edit2.php?id=<?php echo $row['id']; ?>" class="update-button" style="color:#fff;">Update</a>
    ...
    <h1>303 Unit Log</h1>
                                    ...
                                    <button type="button" class="logout-all-button" onclick="confirmLogoutAll()">Logout All Records</button>
                                                ...
                                                <button type="button" class="clear-all-button" id="clearAllButton" onclick="moveTo303Restore()">Clear Table</button>
